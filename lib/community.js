...
const PRIVATE_UNLOCK_MS = 15 * 60 * 1000;
const TRANSLATION_LOCK_MS = 2 * 24 * 60 * 60 * 1000;
const BETA_AVATAR_MAX_BYTES = 96 * 1024;
const ACCENTS = new Set(["violet", "mint", "blue", "rose", "amber"]);
const BETA_ACCENTS = new Set(["cosmic", "electric", "inferno", "ocean", "midnight"]);
const AVATARS = new Set(["nova", "orbit", "pixel", "bolt", "wave", "game"]);
const BETA_AVATARS = new Set(["prism", "comet", "signal", "crown", "ghost", "crystal"]);
const BETA_FRAMES = new Set(["none", "aurora", "starlight", "pulse", "neon", "royal"]);
const BETA_LAYOUTS = new Set(["classic", "spotlight", "compact"]);
const BETA_FONTS = new Set(["default", "rounded", "mono", "serif"]);
const BETA_EFFECTS = new Set(["none", "glow", "scanline", "sparkle"]);
const NOTIFICATION_MODES = new Set(["ALL", "IMPORTANT", "CUSTOM", "NONE"]);
const NOTIFICATION_CATEGORY_KEYS = ["security", "chatSafety", "privateMessages", "support", "announcements", "beta", "rewards"];
...
function privateMessageKey(secret = process.env.CHAT_ENCRYPTION_SECRET || process.env.SESSION_PEPPER) {
  if (!secret || String(secret).length < 24) throw new Error("Private chat encryption is not configured.");
  return crypto.createHash("sha256").update(`scriptnovaa-private-chat-v1:${secret}`).digest();
}

function encryptPrivateText(value, secret) {
  const text = cleanText(value, 1000);
  const iv = crypto.randomBytes(12);
  const cipher = crypto.createCipheriv("aes-256-gcm", privateMessageKey(secret), iv);
  const ciphertext = Buffer.concat([cipher.update(text, "utf8"), cipher.final()]);
  return {encryptedVersion: 1, ciphertext: ciphertext.toString("base64url"),
    encryptionIv: iv.toString("base64url"), encryptionTag: cipher.getAuthTag().toString("base64url")};
}

function decryptPrivateText(message, secret) {
  if (!message?.encryptedVersion) return cleanText(message?.text, 1000);
  try {
    const decipher = crypto.createDecipheriv("aes-256-gcm", privateMessageKey(secret),
        Buffer.from(String(message.encryptionIv || ""), "base64url"));
    decipher.setAuthTag(Buffer.from(String(message.encryptionTag || ""), "base64url"));
    return cleanText(Buffer.concat([
      decipher.update(Buffer.from(String(message.ciphertext || ""), "base64url")), decipher.final(),
    ]).toString("utf8"), 1000);
  } catch (_) {
    return "[This encrypted message could not be opened.]";
  }
}

const SNOVA_SYLLABLES = ["za", "ve", "ki", "ro", "nu", "ta", "xe", "mi", "lo", "sa", "py", "du", "fi", "go", "ha", "ji"];
function scriptNovaEncode(value) {
  return [...String(value || "")].map((character) => {
    const code = character.codePointAt(0);
    return `${SNOVA_SYLLABLES[(code >> 12) & 15]}${SNOVA_SYLLABLES[(code >> 8) & 15]}${SNOVA_SYLLABLES[(code >> 4) & 15]}${SNOVA_SYLLABLES[code & 15]}`;
  }).join("-");
}
function scriptNovaDecode(value) {
  const reverse = new Map(SNOVA_SYLLABLES.map((part, index) => [part, index]));
  return String(value || "").split(/\s*-\s*/).filter(Boolean).map((word) => {
    const parts = String(word).match(/.{2}/g) || [];
    if (parts.length !== 4 || parts.some((part) => !reverse.has(part))) throw new Error("That is not valid ScriptNova Language text.");
    return String.fromCodePoint(parts.reduce((total, part) => (total << 4) | reverse.get(part), 0));
  }).join("");
}
...
function cleanProfileSongUrl(value) {
  const text = String(value || "").trim();
  if (!text) return "";
  let parsed;
  try { parsed = new URL(text); } catch (_) { throw new Error("Use a valid direct HTTPS MP3 link."); }
  const host = parsed.hostname.toLowerCase();
  if (parsed.protocol !== "https:" || parsed.username || parsed.password || host === "localhost" ||
      host.endsWith(".local") || /^\d{1,3}(?:\.\d{1,3}){3}$/.test(host) || host.includes(":")) {
    throw new Error("Profile music must use a public HTTPS address.");
  }
  if (!parsed.pathname.toLowerCase().endsWith(".mp3")) throw new Error("Profile music must be a direct .mp3 link.");
  return parsed.toString().slice(0, 500);
}

function notificationCategory(typeValue) {
  const type = String(typeValue || "").toUpperCase();
  if (type.includes("LOGIN") || type.includes("SECURITY") || type.includes("ACCOUNT")) return "security";
  if (type.includes("CHAT_WARNING") || type.includes("CHAT_MODERATION") || type.includes("CHAT_BAN") ||
      type.includes("CHAT_RESTORED") || type.includes("CHAT_APPEAL")) return "chatSafety";
  if (type.includes("PRIVATE_MESSAGE")) return "privateMessages";
  if (type.includes("SUPPORT") || type.includes("TICKET") || type.includes("APPEAL")) return "support";
  if (type.includes("ANNOUNCEMENT")) return "announcements";
  if (type.includes("BETA") || type.includes("DEVELOPER")) return "beta";
  return "rewards";
}

function notificationPreferences(stored = {}) {
  const mode = NOTIFICATION_MODES.has(String(stored.notificationMode || "").toUpperCase()) ?
    String(stored.notificationMode).toUpperCase() : "ALL";
  const supplied = stored.notificationCategories || {};
  const categories = {};
  NOTIFICATION_CATEGORY_KEYS.forEach((key) => { categories[key] = supplied[key] !== false; });
  return {mode, categories};
}

function notificationAllowed(item, preferences) {
  ...
  if (prefs.mode === "IMPORTANT") return ["security", "chatSafety", "support"].includes(category);
...
function pointRecognition(value) {
  ...
  const milestones = [[25, "RISING"], [50, "SPARK"], [100, "CENTURY"], [250, "ORBIT"],
    [500, "NOVA500"], [1000, "POINTSTAR"], [2500, "ELITE"], [5000, "SUPERNOVA"], [10000, "GALAXY"]];
  const badges = milestones.filter(([minimum]) => points >= minimum).map(([, badge]) => badge);
  const label = badges.at(-1) || "MEMBER";
  const tier = points >= 10000 ? "galaxy" : points >= 5000 ? "supernova" : points >= 2500 ? "elite" :
    points >= 1000 ? "legendary" : points >= 500 ? "rare" : points >= 250 ? "orbit" :
      points >= 100 ? "century" : points >= 50 ? "spark" : points >= 25 ? "rising" : "standard";
  return {points, tier, label, badges};
}
...
function mountCommunity(app, d) {
  const {route, root, read, id, rateLimit, requireAccount, requireAdmin, adminAudit, fail, etagTransaction, atomic,
    verifies, credential} = d;
  async function profileFor(accountId, accountData) {
    ...
    const notificationPrefs = notificationPreferences(stored);
    ...
    return {accountId, username: String(account.username || ""), displayName: cleanDisplayName(stored.displayName, account.username),
      ...
      betaLayout: isBeta && BETA_LAYOUTS.has(stored.betaLayout) ? stored.betaLayout : "classic",
      betaFont: isBeta && BETA_FONTS.has(stored.betaFont) ? stored.betaFont : "default",
      betaEffect: isBeta && BETA_EFFECTS.has(stored.betaEffect) ? stored.betaEffect : "none",
      profileSongUrl: isBeta ? (() => { try { return cleanProfileSongUrl(stored.profileSongUrl); } catch (_) { return ""; } })() : "",
      profileSongTitle: isBeta ? cleanText(stored.profileSongTitle, 60) : "",
      profileSongAutoplay: isBeta && stored.profileSongAutoplay === true,
      profileSongLoop: isBeta && stored.profileSongLoop === true,
      ...
      browserNotifications: stored.browserNotifications === true, notificationMode: notificationPrefs.mode,
      notificationCategories: notificationPrefs.categories, autoTranslateChat: stored.autoTranslateChat !== false,
      chatLanguage: String(stored.chatLanguage || "AUTO").slice(0, 20),
      deletePinEnabled: Boolean(account.deletePinCredential),
      translationBlockedUntil: Number(account.translationBlockedUntil || 0),
      badges, createdAt: Number(account.createdAt || 0),
  ...
  const visibleProfile = (p) => p && ({accountId: p.accountId, username: p.username, displayName: p.displayName,
    ...
    betaStatus: p.betaStatus, betaFrame: p.betaFrame, betaLayout: p.betaLayout,
    betaFont: p.betaFont, betaEffect: p.betaEffect, profileSongUrl: p.profileSongUrl,
    profileSongTitle: p.profileSongTitle, profileSongAutoplay: p.profileSongAutoplay,
    profileSongLoop: p.profileSongLoop, pointBalance: p.pointBalance,
  ...
  async function privateUnlock(account) {
    const unlock = await read(`privateChatUnlocks/${account.loginId}`);
    if (!unlock || unlock.accountId !== account.id || Number(unlock.expiresAt || 0) <= Date.now()) {
      fail("Enter your account PIN to unlock private messages.", 401, "PRIVATE_CHAT_LOCKED");
    }
    return unlock;
  }
  const publicMessage = (messageId, m) => ({messageId, accountId: m.accountId, username: String(m.username || "Unknown"),
    ...
    avatarImage: safeAvatarDataUrl(m.avatarImage || ""), badges: Array.isArray(m.badges) ? m.badges.slice(0, 16) : [],
  ...
  app.get("/api/community/summary", route(async (req, res) => {
    ...
    const notes = await read(`notifications/${a.id}`) || {}; res.json({ok: true, profile, unreadCount,
      notificationCount: Object.values(notes).filter((n) => !n.readAt && notificationAllowed(n, {mode: profile.notificationMode, categories: profile.notificationCategories})).length});
  }));
  ...
  app.patch("/api/profile/me", route(async (req, res) => {
    ...
    const betaLayout = String(req.body.betaLayout || "classic").toLowerCase();
    const betaFont = String(req.body.betaFont || "default").toLowerCase();
    const betaEffect = String(req.body.betaEffect || "none").toLowerCase();
    if (!BETA_LAYOUTS.has(betaLayout) || !BETA_FONTS.has(betaFont) || !BETA_EFFECTS.has(betaEffect) ||
        (!old.betaAccess && (betaLayout !== "classic" || betaFont !== "default" || betaEffect !== "none"))) {
      fail("Choose available Beta appearance options.", 403);
    }
    let profileSongUrl = old.profileSongUrl || "";
    if (Object.hasOwn(req.body, "profileSongUrl")) {
      try { profileSongUrl = cleanProfileSongUrl(req.body.profileSongUrl); } catch (error) { fail(error.message); }
    }
    if (profileSongUrl && !old.betaAccess) fail("Profile music is a Beta feature.", 403);
    const profileSongTitle = Object.hasOwn(req.body, "profileSongTitle") ? cleanText(req.body.profileSongTitle, 60) : old.profileSongTitle;
    const notificationMode = String(req.body.notificationMode || old.notificationMode || "ALL").toUpperCase();
    if (!NOTIFICATION_MODES.has(notificationMode)) fail("Choose a valid notification level.");
    const requestedCategories = req.body.notificationCategories || old.notificationCategories || {};
    const notificationCategories = {};
    NOTIFICATION_CATEGORY_KEYS.forEach((key) => { notificationCategories[key] = requestedCategories[key] !== false; });
    await root.child(`profiles/${a.id}`).update({displayName, bio: cleanText(req.body.bio, 160), accent, avatarId, avatarImage,
      ...
      betaLayout: old.betaAccess ? betaLayout : "classic", betaFont: old.betaAccess ? betaFont : "default",
      betaEffect: old.betaAccess ? betaEffect : "none",
      profileSongUrl: old.betaAccess ? profileSongUrl : "", profileSongTitle: old.betaAccess ? profileSongTitle : "",
      profileSongAutoplay: old.betaAccess && (Object.hasOwn(req.body, "profileSongAutoplay") ? req.body.profileSongAutoplay === true : old.profileSongAutoplay),
      profileSongLoop: old.betaAccess && (Object.hasOwn(req.body, "profileSongLoop") ? req.body.profileSongLoop === true : old.profileSongLoop),
      ...
      notificationMode, notificationCategories,
  ...
  app.get("/api/community/private", route(async (req, res) => {
    ...
    const previewUpdates = {}; rows.forEach(([threadId, thread]) => { if (thread.lastMessagePreview && thread.lastMessagePreview !== "Encrypted private message") previewUpdates[`communityPrivateThreads/${threadId}/lastMessagePreview`] = "Encrypted private message"; }); if (Object.keys(previewUpdates).length) await root.update(previewUpdates);
    const threads = await Promise.all(rows.map(async ([threadId, t]) => { const otherId = Object.keys(t.members).find((key) => key !== a.id); const p = await profileFor(otherId); return {threadId, other: {...visibleProfile(p), presence: await presenceFor(otherId, p)}, preview: t.lastSenderAccountId ? "Encrypted private message" : "", unread: t.lastSenderAccountId !== a.id && Number(t.updatedAt || 0) > Number(t.lastReadAt?.[a.id] || 0)}; })); res.json({ok: true, threads});
  }));
  app.post("/api/community/private/unlock", route(async (req, res) => {
    const a = await requireAccount(req); await requireChat(a); await rateLimit(a.id, "PRIVATE_CHAT_UNLOCK", 12, 60 * 60);
    const supplied = String(req.body.pin || "");
    let mode = "FULL";
    if (a.data.deletePinCredential && verifies(supplied, a.data.deletePinCredential, "PIN_PEPPER")) {
      mode = "DECOY";
      await root.child(`accounts/${a.id}`).update({translationBlockedUntil: Date.now() + TRANSLATION_LOCK_MS,
        deletePinLastUsedAt: Date.now(), updatedAt: Date.now()});
    } else if (!verifies(supplied, a.data.pinCredential, "PIN_PEPPER")) {
      fail("The PIN is incorrect.", 401, "PRIVATE_CHAT_PIN_INVALID");
    }
    const expiresAt = Date.now() + PRIVATE_UNLOCK_MS;
    await root.child(`privateChatUnlocks/${a.loginId}`).set({accountId: a.id, mode, expiresAt, createdAt: Date.now()});
    res.json({ok: true, mode, expiresAt, translationBlockedUntil: mode === "DECOY" ? Date.now() + TRANSLATION_LOCK_MS : Number(a.data.translationBlockedUntil || 0)});
  }));
  app.post("/api/community/private/lock", route(async (req, res) => {
    const a = await requireAccount(req); await root.child(`privateChatUnlocks/${a.loginId}`).remove(); res.json({ok: true});
  }));
  ...
  app.get("/api/community/private/:threadId/messages", route(async (req, res) => {
    const {account, thread} = await threadMember(req, req.params.threadId); const unlock = await privateUnlock(account);
    const values = await read(`communityPrivateMessages/${req.params.threadId}`) || {}; const migrationUpdates = {};
    const messages = Object.entries(values).sort((x, y) => Number(x[1].createdAt) - Number(y[1].createdAt)).slice(-100).map(([messageId, m]) => {
      const text = decryptPrivateText(m); const {ciphertext, encryptionIv, encryptionTag, ...safe} = m;
      if (!m.encryptedVersion && m.text && !m.deleted) {
        const encrypted = encryptPrivateText(text); Object.entries(encrypted).forEach(([key, value]) => { migrationUpdates[`communityPrivateMessages/${req.params.threadId}/${messageId}/${key}`] = value; });
        migrationUpdates[`communityPrivateMessages/${req.params.threadId}/${messageId}/text`] = null;
      }
      return {messageId, ...safe, text: unlock.mode === "DECOY" && !m.deleted ? scriptNovaEncode(text) : text,
        scriptNovaLanguage: unlock.mode === "DECOY"};
    });
    if (Object.keys(migrationUpdates).length) await root.update(migrationUpdates);
    const people = activeTyping(await read(`communityTyping/private/${req.params.threadId}`), Date.now(), account.id);
    res.json({ok: true, messages, unlockMode: unlock.mode, unlockExpiresAt: unlock.expiresAt,
      typingLabel: typingLabel(people), lastReadAt: thread.lastReadAt || {}});
  }));
  app.post("/api/community/private/:threadId/messages", route(async (req, res) => {
    const {account, thread} = await threadMember(req, req.params.threadId); const unlock = await privateUnlock(account); if (unlock.mode !== "FULL") fail("Enter your account PIN to send private messages.", 403, "PRIVATE_CHAT_DECOY"); await rateLimit(account.id, "COMMUNITY_PRIVATE_MESSAGE", 40, 60); const otherId = Object.keys(thread.members).find((key) => key !== account.id); const target = await profileFor(otherId); if (!target.allowDirectMessages || await read(`communityBlocks/${otherId}/${account.id}`)) fail("This user is not accepting private messages.", 403);
    ...
    const stored = {senderAccountId: account.id, senderUsername: account.data.username, senderDisplayName: p.displayName, senderAvatarId: p.avatarId, senderAvatarImage: p.avatarImage, senderBadges: p.badges, senderPointTier: p.pointTier, senderBetaFrame: p.betaFrame, ...encryptPrivateText(text), createdAt: now, readBy: {[account.id]: now}};
    await root.update({[`communityPrivateMessages/${req.params.threadId}/${messageId}`]: stored, [`communityPrivateThreads/${req.params.threadId}/updatedAt`]: now, [`communityPrivateThreads/${req.params.threadId}/lastSenderAccountId`]: account.id, [`communityPrivateThreads/${req.params.threadId}/lastMessagePreview`]: "Encrypted private message", [`communityPrivateThreads/${req.params.threadId}/lastReadAt/${account.id}`]: now, [`communityTyping/private/${req.params.threadId}/${account.id}`]: null, [`notifications/${otherId}/${noteId}`]: {type: "PRIVATE_MESSAGE", title: `Message from ${p.displayName}`, message: "Open Chat and enter your PIN to view this encrypted message.", createdAt: now, readAt: null}}); res.status(201).json({ok: true, message: {messageId, ...stored, ciphertext: undefined, encryptionIv: undefined, encryptionTag: undefined, text}});
  }));
  app.patch("/api/community/private/:threadId/messages/:messageId", route(async (req, res) => { const {account} = await threadMember(req, req.params.threadId); const unlock = await privateUnlock(account); if (unlock.mode !== "FULL") fail("Enter your account PIN to edit private messages.", 403); const path = `communityPrivateMessages/${req.params.threadId}/${req.params.messageId}`; const item = await read(path); if (!item || item.senderAccountId !== account.id) fail("Message not found.", 404); if (Date.now() - Number(item.createdAt) > EDIT_WINDOW_MS) fail("Messages can only be edited for two minutes.", 409); const text = cleanText(req.body.text, 1000); await validatePost(account, text, "PRIVATE_EDIT"); await root.child(path).update({...encryptPrivateText(text), text: null, editedAt: Date.now()}); res.json({ok: true}); }));
  app.delete("/api/community/private/:threadId/messages/:messageId", route(async (req, res) => { const {account} = await threadMember(req, req.params.threadId); await privateUnlock(account); const path = `communityPrivateMessages/${req.params.threadId}/${req.params.messageId}`; const item = await read(path); if (!item || item.senderAccountId !== account.id) fail("Message not found.", 404); await root.child(path).update({text: null, ciphertext: null, encryptionIv: null, encryptionTag: null, deleted: true, deletedAt: Date.now()}); res.json({ok: true}); }));
  ...
  app.post("/api/community/translate", route(async (req, res) => {
    ...
    if (Number(a.data.translationBlockedUntil || 0) > Date.now()) {
      fail(`Translation is unavailable until ${new Date(a.data.translationBlockedUntil).toISOString()}.`, 403, "TRANSLATION_TEMPORARILY_LOCKED");
    }
  ...
  app.get("/api/community/translation-access", route(async (req, res) => {
    const a = await requireAccount(req); const blockedUntil = Number(a.data.translationBlockedUntil || 0);
    res.json({ok: true, allowed: blockedUntil <= Date.now(), blockedUntil});
  }));
  app.post("/api/community/scriptnova-language", route(async (req, res) => {
    const a = await requireAccount(req); await rateLimit(a.id, "SCRIPTNOVA_LANGUAGE", 60, 60 * 60);
    if (Number(a.data.translationBlockedUntil || 0) > Date.now()) {
      fail(`ScriptNova Language tools are unavailable until ${new Date(a.data.translationBlockedUntil).toISOString()}.`, 403, "TRANSLATION_TEMPORARILY_LOCKED");
    }
    const action = String(req.body.action || "ENCODE").toUpperCase(); const text = cleanText(req.body.text, 4000);
    if (!text) fail("Enter text first.");
    let output;
    try { output = action === "DECODE" ? scriptNovaDecode(text) : scriptNovaEncode(text); } catch (error) { fail(error.message); }
    res.json({ok: true, action: action === "DECODE" ? "DECODE" : "ENCODE", output});
  }));
  ...
  app.get("/api/notifications", route(async (req, res) => { const a = await requireAccount(req); const profile = await profileFor(a.id, a.data); const values = await read(`notifications/${a.id}`) || {}; const preferences = {mode: profile.notificationMode, categories: profile.notificationCategories}; res.json({ok: true, preferences, notifications: Object.entries(values).filter(([, n]) => notificationAllowed(n, preferences)).map(([notificationId, n]) => ({notificationId, ...n, category: notificationCategory(n.type)})).sort((x, y) => Number(y.createdAt) - Number(x.createdAt)).slice(0, 50)}); }));
  app.post("/api/notifications/read", route(async (req, res) => { const a = await requireAccount(req); const profile = await profileFor(a.id, a.data); const values = await read(`notifications/${a.id}`) || {}; const preferences = {mode: profile.notificationMode, categories: profile.notificationCategories}; const updates = {}; Object.entries(values).filter(([, item]) => notificationAllowed(item, preferences)).forEach(([key]) => { updates[`notifications/${a.id}/${key}/readAt`] = Date.now(); }); if (Object.keys(updates).length) await root.update(updates); res.json({ok: true}); }));
...
module.exports = {mountCommunity, cleanText, cleanDisplayName, conversationKey, activeTyping, typingLabel,
  ...
  chatUnbanEligibility, cleanAvatarDataUrl, safeAvatarDataUrl, cleanProfileSongUrl, betaAccess, pointRecognition,
  notificationCategory, notificationPreferences, notificationAllowed, translateWithGemini,
  encryptPrivateText, decryptPrivateText, scriptNovaEncode, scriptNovaDecode,
  PUBLIC_RETENTION_MS, EDIT_WINDOW_MS, CHAT_BAN_MS, CHAT_UNBAN_FEE, PRIVATE_UNLOCK_MS, TRANSLATION_LOCK_MS};
