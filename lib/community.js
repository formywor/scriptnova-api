...
function mountCommunity(app, d) {
  ...
  app.post("/api/admin/community/chat-status", route(async (req, res) => {
    const admin = await requireAdmin(req, "ADMIN"); const username = String(req.body.username || "").trim().toLowerCase();
    ...
    const action = String(req.body.action || "").toUpperCase(); const reason = cleanText(req.body.reason, 300);
    ...
    if (action === "BAN") { durationHours = Math.max(1, Math.min(720, Number(req.body.durationHours || 48))); bannedUntil = now + durationHours * 60 * 60 * 1000; }
    ...
    res.json({ok: true, action, bannedUntil});
  }));
}
...
module.exports = {mountCommunity, cleanText, cleanDisplayName, conversationKey, activeTyping, typingLabel,
  ...
  chatUnbanEligibility, PUBLIC_RETENTION_MS, EDIT_WINDOW_MS, CHAT_BAN_MS, CHAT_UNBAN_FEE};
