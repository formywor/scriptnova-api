...
function mountCommunity(app, d) {
  ...
  async function profileFor(accountId, accountData) {
    ...
    return {accountId, username: String(account.username || ""), displayName: cleanDisplayName(stored.displayName, account.username),
      ...
      avatarId: AVATARS.has(stored.avatarId) ? stored.avatarId : "nova",
  ...
  const visibleProfile = (p) => p && ({accountId: p.accountId, username: p.username, displayName: p.displayName,
    ...
    betaStatus: p.betaStatus, createdAt: p.createdAt});
  ...
  const publicMessage = (messageId, m) => ({messageId, accountId: m.accountId, username: String(m.username || "Unknown"),
    ...
    avatarImage: safeAvatarDataUrl(m.avatarImage || ""), badges: Array.isArray(m.badges) ? m.badges.slice(0, 6) : [],
  ...
  app.patch("/api/profile/me", route(async (req, res) => {
    ...
    if (displayName.length < 2 || !(ACCENTS.has(accent) || (old.betaAccess && BETA_ACCENTS.has(accent))) || !AVATARS.has(avatarId)) fail("Choose an available profile style.");
    ...
    await root.child(`profiles/${a.id}`).update({displayName, bio: cleanText(req.body.bio, 160), accent, avatarId, avatarImage,
      betaStatus: old.betaAccess ? betaStatus : "",
  ...
  app.post("/api/community/public/messages", route(async (req, res) => {
    ...
    const stored = {accountId: a.id, username: a.data.username, displayName: p.displayName, avatarId: p.avatarId, avatarImage: p.avatarImage, badges: p.badges, text, createdAt: now, deleteAfter: now + PUBLIC_RETENTION_MS};
  ...
  app.post("/api/community/private/:threadId/messages", route(async (req, res) => {
    ...
    const stored = {senderAccountId: account.id, senderUsername: account.data.username, senderDisplayName: p.displayName, senderAvatarId: p.avatarId, senderAvatarImage: p.avatarImage, senderBadges: p.badges, text, createdAt: now, readBy: {[account.id]: now}};
...
module.exports = {mountCommunity, cleanText, cleanDisplayName, conversationKey, activeTyping, typingLabel,
  ...
  chatUnbanEligibility, cleanAvatarDataUrl, safeAvatarDataUrl, betaAccess, translateWithGemini,
