...
module.exports = function mount(app, deps) {
  ...
  function records(site) {
    ...
    const relative = name => site.dnsZone && name.endsWith(`.${site.dnsZone}`) ? name.slice(0,-site.dnsZone.length-1) : name === site.dnsZone ? "@" : null;
    const result=[{type:"TXT",name:`_scriptnovaa.${site.customDomain}`,host:relative(`_scriptnovaa.${site.customDomain}`),value:`scriptnovaa-verification=${site.verificationToken}`}];
    if(site.domainKind !== "ROOT") result.push({type:"CNAME",name:site.customDomain,host:relative(site.customDomain),value:process.env.HOSTING_TARGET_DOMAIN || "Not configured by ScriptNovaa yet"});
    return result;
  }
  ...
  app.post("/api/developer/site", route(async (req, res) => {
    ...
    const domainKind=customDomain ? (req.body.domainKind === "ROOT" ? "ROOT" : "SUBDOMAIN") : "SCRIPTNOVAA";
    const dnsZone=customDomain && req.body.dnsZone ? domain(req.body.dnsZone) : "";
    if(dnsZone && (domainKind === "ROOT" ? dnsZone !== customDomain : !customDomain.endsWith(`.${dnsZone}`))) reject("The DNS zone must be the root domain you manage. A subdomain must end with that domain.");
    ...
    await atomic(data => {
      ...
      data.hostingSites[user.id] = {accountId: user.id, username: user.data.username, slug: name, customDomain, domainKind, dnsZone, verificationToken, domainVerifiedAt: 0,
  ...
  app.post("/api/developer/domain/verify", route(async (req, res) => {
    ...
    const problems = [];
    if (!txt.some(x => x.join("") === `scriptnovaa-verification=${site.verificationToken}`)) problems.push(`Ownership TXT missing or incorrect at _scriptnovaa.${site.customDomain}. If your DNS provider appends the zone name, use the relative Host rather than the full DNS name. Keep the full verification value.`);
    if (site.domainKind !== "ROOT" && !names.some(x => x.toLowerCase().replace(/\.$/, "") === process.env.HOSTING_TARGET_DOMAIN.toLowerCase().replace(/\.$/, ""))) problems.push(`Routing CNAME missing or incorrect at ${site.customDomain}; expected ${process.env.HOSTING_TARGET_DOMAIN}. Use DNS-only mode if your provider proxies records. Root or flattened routing requires staff review.`);
    if (problems.length) reject(`${problems.join(" ")} If you just saved the correct records, allow DNS caches to update and retry.`);
