...
function slug(value) {
  ...
  if (!/^[a-z][a-z0-9-]{1,30}[a-z0-9]$/.test(text) || RESERVED.has(text)) reject("Choose a unique 3–32 character address: letters, numbers and internal hyphens.");
  return text;
}
...
module.exports = function mount(app, deps) {
  ...
  app.post("/api/developer/site", route(async (req, res) => {
    ...
    await atomic(data => {
      ...
      data.hostingSites[user.id] = {accountId: user.id, username: user.data.username, slug: name, customDomain, verificationToken, domainVerifiedAt: 0, status: "PENDING", template: "scriptnovaa-demo", createdAt: Date.now()};
      return data;
    }); res.json({ok: true});
  }));
