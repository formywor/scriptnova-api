...
module.exports = function mount(app, deps) {
  ...
  app.post("/api/developer/domain/verify", route(async (req, res) => {
    ...
    const problems = [];
    if (!txt.some(x => x.join("") === `scriptnovaa-verification=${site.verificationToken}`)) problems.push(`Ownership TXT missing or incorrect at _scriptnovaa.${site.customDomain}. In Namecheap, remove your managed domain from the Host field; keep the full verification value.`);
    if (!names.some(x => x.toLowerCase().replace(/\.$/, "") === process.env.HOSTING_TARGET_DOMAIN.toLowerCase().replace(/\.$/, ""))) problems.push(`Routing CNAME missing or incorrect at ${site.customDomain}; expected ${process.env.HOSTING_TARGET_DOMAIN}. A/AAAA records or flattened ALIAS records do not satisfy this check. Use a subdomain to preserve an existing root website.`);
