...
const BETA_PAIRING_LIMIT = 4;
const BETA_PAIRING_RECHARGE_MS = 28 * 24 * 60 * 60 * 1000;

function betaPairingUsage(account, now = Date.now()) {
  const usedAt = (Array.isArray(account?.betaConnectionCodeUses) ?
    account.betaConnectionCodeUses : []).map(Number)
      .filter((value) => Number.isFinite(value) && value > now - BETA_PAIRING_RECHARGE_MS)
      .sort((a, b) => a - b).slice(-BETA_PAIRING_LIMIT);
  return {usedAt, available: Math.max(0, BETA_PAIRING_LIMIT - usedAt.length),
    nextRechargeAt: usedAt.length ? usedAt[0] + BETA_PAIRING_RECHARGE_MS : null};
}
...
function complete(data, input) {
  ...
  const betaQuota = pairing.betaReplacement === true ? betaPairingUsage(account, now) : null;
  if (betaQuota && betaQuota.available <= 0) {
  ...
  if (betaQuota) {
    account.betaConnectionCodeUses = [...betaQuota.usedAt, now];
    pairing.betaQuotaConsumedAt = now;
  }
...
module.exports = {complete, resolveHash, betaPairingUsage,
  BETA_PAIRING_LIMIT, BETA_PAIRING_RECHARGE_MS};
