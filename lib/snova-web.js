...
function xmlValue(item, tag) {
  ...
  return plainText(String(match?.[1] || "").replace(/^<!\[CDATA\[|\]\]>$/g, ""));
}

function parseWebResults(xml) {
  const results = [];
  const items = String(xml || "").match(/<item(?:\s[^>]*)?>[\s\S]*?<\/item>/gi) || [];
  for (const item of items.slice(0, 12)) {
    const url = safeWebUrl(xmlValue(item, "link"));
    const title = xmlValue(item, "title").slice(0, 240);
    if (!url || !title) continue;
    results.push({title, url, snippet: xmlValue(item, "description").slice(0, 600)});
  }
  return results;
}

async function searchWeb(query, fetcher = fetchWithTimeout) {
  const url = new URL(BING_RSS);
  url.searchParams.set("format", "rss");
  url.searchParams.set("q", query);
  const response = await fetcher(url, {headers: {"User-Agent": "ScriptNovaa-Snova/1.1 (https://scriptnovaa.com/)"}});
  if (!response.ok) throw new Error("Web search is temporarily unavailable.");
  const xml = await response.text();
  if (Buffer.byteLength(xml, "utf8") > 500000) throw new Error("Web search returned too much data.");
  return parseWebResults(xml);
}

function decodeDuckUrl(value) {
  const safe = safeWebUrl(String(value || "").replace(/&amp;/g, "&"));
  if (!safe) return "";
  try {
    const url = new URL(safe);
    const target = url.hostname.endsWith("duckduckgo.com") && url.searchParams.get("uddg");
    return target ? safeWebUrl(decodeURIComponent(target)) : safe;
  } catch (_error) {
    return safe;
  }
}
...
function parseDuckDuckGoResults(html) {
  ...
  return links.slice(0, 12).map((match) => ({
    url: decodeDuckUrl(match[1]),
    title: plainText(match[2]).slice(0, 240),
    snippet: "Result supplied through DuckDuckGo's lightweight search page.",
  })).filter((item) => item.url && item.title);
}

async function searchDuckDuckGo(query, fetcher = fetchWithTimeout) {
  const response = await fetcher(DUCKDUCKGO_HTML, {
    method: "POST",
    headers: {"Content-Type": "application/x-www-form-urlencoded",
      "User-Agent": "ScriptNovaa-Snova/1.2 (https://scriptnovaa.com/)"},
    body: new URLSearchParams({q: query}).toString(),
  });
  if (!response.ok) throw new Error("DuckDuckGo search is temporarily unavailable.");
  const html = await response.text();
  if (Buffer.byteLength(html, "utf8") > 1000000) throw new Error("Search returned too much data.");
  return parseDuckDuckGoResults(html);
}

async function searchPreferred(query) {
  try {
...
function scriptNovaaResults(query) {
  if (!/(script\s*novaa?|share\s*browser|project\s*z|galaxy\s*browser)/i.test(query)) return [];
  return [
    {title: "ScriptNovaa", url: "https://scriptnovaa.com/", snippet: "The official home of ScriptNovaa products."},
    {title: "Share Browser by ScriptNovaa", url: "https://scriptnovaa.com/share-browser", snippet: "Download and learn about Share Browser."},
    {title: "Project Z by ScriptNovaa", url: "https://scriptnovaa.com/project-z", snippet: "Project Z downloads, requirements and product information."},
    {title: "Galaxy Browser by ScriptNovaa", url: "https://scriptnovaa.com/galaxy-browser", snippet: "Galaxy Browser downloads, Snova search, partner demonstrations and Galaxy token information."},
  ];
}
...
function searchResultsHtml(query, results, access = "", webResults = []) {
  ...
  const seen = new Set();
  const webCards = [...scriptNovaaResults(query), ...webResults].filter((result) => {
    ...
    let host = "Website";
    try { host = new URL(result.url).hostname.replace(/^www\./, ""); } catch (_error) {}
    return `<article class="card"><h2><a href="${escapeHtml(result.url)}">${escapeHtml(result.title)}</a></h2><p class="muted">${escapeHtml(plainText(result.snippet))}</p><a class="button" href="${escapeHtml(result.url)}">Open website</a> <span class="muted">${escapeHtml(host)} · For modern games or apps, open the result and use Project Z’s Modern button.</span></article>`;
  });
  const readerCards = results.map((result) => {
  ...
  const cards = [...webCards, ...readerCards].join("");
  if (!cards) return `<h1>Snova Search</h1><p class="muted">No web results were found. Try another phrase or one of these search providers.</p>${shortcuts}`;
  return `<h1>Results for “${escapeHtml(query)}”</h1><p class="notice">Snova now searches the wider web. Wikipedia pages also have a lightweight Reader. JavaScript-heavy games and web applications need Project Z’s Modern button because the HTA’s built-in engine is older. Snova Reader never runs page scripts or starts downloads.</p>${shortcuts}${cards}`;
}
...
async function requireSnovaAccess(rawAccess, dependencies, now = Date.now()) {
  ...
  if (!/^(?:z|galaxy)_[a-f0-9]{40}$/.test(sessionId) || !/^[a-f0-9]{64}$/.test(secret)) {
    throw Object.assign(new Error("Start an active Project Z or Galaxy token to use Snova."), {statusCode: 403});
  }
  const session = await dependencies.read(`sessions/${sessionId}`);
  if (!session || !["z", "galaxy"].includes(session.product) || session.status !== "ACTIVE" ||
      Number(session.expiresAt || 0) <= now || Number(session.leaseExpiresAt || 0) <= now ||
      session.webAccessHash !== dependencies.hmac(secret)) {
    throw Object.assign(new Error("This Snova access has finished. Start another browser token."), {statusCode: 403});
  }
...
function sanitizeWikipediaHtml(html, articleUrl = "https://en.wikipedia.org/wiki/Main_Page", access = "") {
  const allowedTags = new Set(["h1", "h2", "h3", "h4", "h5", "h6", "p", "div", "span",
  ...
  let source = String(html || "")
      .replace(/<!--[\s\S]*?-->/g, "")
      .replace(/<(script|style|textarea|option|noscript|iframe|object|embed|form)[^>]*>[\s\S]*?<\/\1\s*>/gi, "")
      .replace(/<![^>]*>/g, "");
  return source.replace(/<\/?([a-z][a-z0-9]*)\b([^>]*)>/gi, (whole, rawTag, rawAttributes) => {
    const tag = rawTag.toLowerCase();
    if (!allowedTags.has(tag)) return "";
    if (/^<\//.test(whole)) return voidTags.has(tag) ? "" : `</${tag}>`;
    const attributes = Object.create(null);
    String(rawAttributes || "").replace(/([a-z][a-z0-9:-]*)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'=<>`]+))/gi,
        (_match, name, doubleQuoted, singleQuoted, unquoted) => {
          attributes[name.toLowerCase()] = doubleQuoted ?? singleQuoted ?? unquoted ?? "";
          return "";
        });
    const output = [];
    if (tag === "a") {
      const href = resolveArticleLink(attributes.href, articleUrl, access);
      if (href) output.push(`href="${escapeHtml(href)}"`);
      if (attributes.title) output.push(`title="${escapeHtml(attributes.title.slice(0, 240))}"`);
    } else if (tag === "img") {
      const src = resolveArticleImage(attributes.src, articleUrl);
      if (!src) return "";
      output.push(`src="${escapeHtml(src)}"`);
      output.push(`alt="${escapeHtml(String(attributes.alt || "Wikipedia image").slice(0, 300))}"`);
      output.push("loading=\"lazy\"");
      if (attributes.title) output.push(`title="${escapeHtml(attributes.title.slice(0, 240))}"`);
      for (const dimension of ["width", "height"]) {
        if (/^[1-9][0-9]{0,4}$/.test(attributes[dimension] || "")) {
          output.push(`${dimension}="${attributes[dimension]}"`);
        }
      }
    } else if (tag === "th" || tag === "td") {
      for (const span of ["colspan", "rowspan"]) {
        if (/^[1-9][0-9]?$/.test(attributes[span] || "")) output.push(`${span}="${attributes[span]}"`);
      }
      if (tag === "th" && /^(row|col|rowgroup|colgroup)$/.test(attributes.scope || "")) {
        output.push(`scope="${attributes.scope}"`);
      }
    }
...
function mountSnovaWeb(app, dependencies) {
  ...
  app.get("/search", route(async (req, res) => {
    ...
    try {
      ...
      return sendHtml(res, shell("Browser session required", `<h1>ScriptNovaa browser session required</h1><p class="muted">${escapeHtml(error.message)}</p><p>Open Project Z or Galaxy Browser and start a compatible token.</p>`), 403);
    }
    ...
    if (!query) {
      return sendHtml(res, shell("Search", "<h1>Snova Search</h1><p class=\"notice\">Galaxy detected. Search queries are relayed to DuckDuckGo with an automatic fallback. Destination websites open directly and do not travel through ScriptNovaa.</p><p class=\"muted\">Wikipedia results can also open in a lightweight reader.</p>", "", authorization.access));
    }
    const [wikipedia, web] = await Promise.allSettled([searchWikipedia(query), searchPreferred(query)]);
    const wikipediaResults = wikipedia.status === "fulfilled" ? wikipedia.value : [];
    const webResults = web.status === "fulfilled" ? web.value : [];
    if (!wikipediaResults.length && !webResults.length && !scriptNovaaResults(query).length) {
      return sendHtml(res, shell("Search", searchUnavailableHtml(query), query, authorization.access));
    }
    return sendHtml(res, shell("Search", searchResultsHtml(query, wikipediaResults, authorization.access, webResults), query, authorization.access));
  }));
  ...
  app.get("/reader", route(async (req, res) => {
    ...
    try {
      ...
      return sendHtml(res, shell("Browser session required", `<h1>ScriptNovaa browser session required</h1><p class="muted">${escapeHtml(error.message)}</p>`), 403);
    }
...
module.exports = {MAX_QUERY_LENGTH, MAX_READER_BYTES, escapeHtml, plainText,
  wikipediaArticleUrl, safeReaderUrl, safeWebUrl, parseWebResults, searchWeb,
  decodeDuckUrl, parseDuckDuckGoResults, searchDuckDuckGo, searchPreferred, scriptNovaaResults,
  searchWikipedia, searchResultsHtml, searchUnavailableHtml,
