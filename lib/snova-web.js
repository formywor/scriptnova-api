...
function scriptNovaaResults(query) {
  if (!/(script\s*novaa?|share\s*browser|project\s*z)/i.test(query)) return [];
  return [
...
function mountSnovaWeb(app, dependencies) {
  ...
  app.get("/search", route(async (req, res) => {
    ...
    if (!query) {
      return sendHtml(res, shell("Search", "<h1>Snova Search</h1><p class=\"muted\">Search the wider web. Wikipedia results can also open in a lightweight reader. Modern games and applications should use Project Z’s Modern button.</p>", "", authorization.access));
    }
    const [wikipedia, web] = await Promise.allSettled([searchWikipedia(query), searchWeb(query)]);
...
module.exports = {MAX_QUERY_LENGTH, MAX_READER_BYTES, escapeHtml, plainText,
  wikipediaArticleUrl, safeReaderUrl, safeWebUrl, parseWebResults, searchWeb, scriptNovaaResults,
