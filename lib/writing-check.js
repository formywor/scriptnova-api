...
function scanInvisibleText(text) {
  ...
  return {
    ...
    total,
    zeroWidthCount,
    possibleEncodedWatermark: zeroWidthCount >= 6,
    counts,
    samples,
  };
}
...
async function copyleaksAnalysis(text, fetcher = fetch, credentials = {}) {
  ...
  try {
    ...
    const response = await fetcher(
        `https://api.copyleaks.com/v2/writer-detector/${scanId}/check`, {
          ...
          headers: {"Content-Type": "application/json",
            Authorization: `Bearer ${copyleaksCredential.token}`},
    ...
    return {name: "Copyleaks", available: false,
      reason: "Copyleaks timed out or could not be reached."};
  } finally {
    clearTimeout(timer);
  }
}
...
async function huggingFaceAnalysis(text, fetcher = fetch, apiKey = process.env.HF_TOKEN) {
  ...
  if (!apiKey) return {name: "Hugging Face", available: false,
    reason: "Hugging Face is not configured."};
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 15000);
  try {
    const response = await fetcher(
    ...
    return {name: "Hugging Face", available: false,
      reason: "Hugging Face timed out or could not be reached."};
  } finally {
    clearTimeout(timer);
  }
}
...
async function analyzeWriting(text, options = {}) {
  ...
  return {
    score,
    verdict: verdictFor(score, style.wordCount),
    ...
    online: {
      ...
      averageScore: externalAverage === null ? null : Math.round(externalAverage),
      providers,
    },
...
async function improveWriting(text, fetcher = fetch, apiKey = process.env.GEMINI_API_KEY) {
  ...
  try {
    const response = await fetcher(
        `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(GEMINI_MODEL)}:generateContent`, {
          ...
          body: JSON.stringify({
            ...
            contents: [{role: "user", parts: [{text: JSON.stringify({
              task: "Revise this writing for clarity and a more natural, specific voice. Preserve quotations, names, numbers, and citations. List the most important edits and any factual claims the author should verify.",
              document: text,
            })}]}],
    ...
    if (!response.ok) return {available: false,
      reason: `Writing improvement was unavailable (${response.status}).`};
    const body = await response.json();
    const output = body?.candidates?.[0]?.content?.parts?.map((part) => part.text || "").join("") || "";
