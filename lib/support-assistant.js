...
function redactSupportSecrets(value) {
  return String(value || "")
      .replace(/\b(?:SHARE|Z)(?:-[A-Z0-9_]{4}){3,}\b/gi, "[browser token removed]")
...
function supportAssistantReply(message, context = {}) {
  ...
  const priorText = (context.previousUserMessages || []).slice(-4).join(" ")
      .toLowerCase().replace(/[’‘]/g, "'");
  const isShortFollowUp = /^(please|i didn'?t do it|i did not do it|why|how long(?: will it last| does it last| is it for)?|what now|that'?s wrong|thats wrong|help me|can you help|what can i do)[.!?\s]*$/i.test(normalized);
  ...
  if (includesAny(normalized, [/nothing happen(?:s|ed)?/, /(?:won'?t|will not) (?:open|launch|start)/, /(?:doesn'?t|does not) (?:open|launch|start)/, /launch(?:er|ing)?/, /browser.*(?:closed|ended|quit|disappeared)/, /closed quickly/, /download.*(?:didn|doesn|not|fail)/, /run.*share browser/, /ended my token/, /token.*(?:ended|expired).*(?:didn|without|unused)/, /chrome/, /edge/])) {
  ...
  if (includesAny(normalized, [/sign in/, /login/, /account access/, /forgot/, /recover/, /reset.*pin/, /change.*pin/])) {
    return {transfer: false, message: "For sign-in problems, verify the username and PIN, then wait before retrying if rate limited. New and updated PINs contain 7–14 digits; an older account may still use its existing shorter PIN until it is changed in Settings. If you forgot the PIN, use the saved recovery code on the recovery page. Never send that recovery code in chat. Without the PIN and recovery code, support cannot prove ownership automatically."};
  }
