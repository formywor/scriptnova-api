...
function mountAccountSecurity(app, dependencies) {
  const {route, root, read, id, requireAccount, rateLimit, verifies, credential,
    code, fail, hmac, validatePin} = dependencies;
  ...
  app.get("/api/account/security", route(async (req, res) => {
    ...
    res.json({ok: true, sessions, recentActivity, alerts,
      deletePinEnabled: Boolean(account.data.deletePinCredential),
  ...
  app.post("/api/account/security/device/remove", route(async (req, res) => {
    ...
    await root.update({
      ...
      [`accounts/${account.id}/registeredDeviceId`]: account.data.secondDeviceId || null,
      [`accounts/${account.id}/secondDeviceId`]: null,
  ...
  app.post("/api/account/security/pin/change", route(async (req, res) => {
    const account = await requireAccount(req);
    await rateLimit(account.id, "SECURITY_PIN_CHANGE", 3, 24 * 60 * 60);
    requirePin(account, req.body.currentPin);
    const newPin = validatePin(req.body.newPin);
    if (String(req.body.currentPin || "") === newPin) {
      fail("Choose a new PIN that is different from the current PIN.");
    }
    const sessions = await read("loginSessions") || {};
    const now = Date.now();
    const updates = {
      [`accounts/${account.id}/pinCredential`]: credential(newPin, "PIN_PEPPER"),
      [`accounts/${account.id}/pinLength`]: newPin.length,
      [`accounts/${account.id}/pinUpgradeRequired`]: false,
      [`accounts/${account.id}/pinChangedAt`]: now,
      [`accounts/${account.id}/updatedAt`]: now,
      [`loginGuards/${account.id}`]: null,
    };
    Object.entries(sessions).forEach(([sessionId, session]) => {
      if (session.accountId === account.id && sessionId !== account.loginId &&
          session.revoked !== true) {
        updates[`loginSessions/${sessionId}/revoked`] = true;
        updates[`loginSessions/${sessionId}/revokedAt`] = now;
        updates[`loginSessions/${sessionId}/revokeReason`] = "PIN_CHANGED";
      }
    });
    await root.update(updates);
    await activity(account.id, "PIN_CHANGED");
    res.json({ok: true, otherSessionsRevoked: true});
  }));

  app.post("/api/account/security/delete-pin", route(async (req, res) => {
    const account = await requireAccount(req);
    await rateLimit(account.id, "SECURITY_DELETE_PIN", 5, 24 * 60 * 60);
    requirePin(account, req.body.currentPin);
    const action = String(req.body.action || "SET").toUpperCase();
    const updates = {updatedAt: Date.now()};
    if (action === "REMOVE") {
      updates.deletePinCredential = null;
      updates.deletePinChangedAt = Date.now();
    } else {
      const deletePin = validatePin(req.body.deletePin);
      if (verifies(deletePin, account.data.pinCredential, "PIN_PEPPER")) {
        fail("The Delete PIN must be different from your account PIN.");
      }
      updates.deletePinCredential = credential(deletePin, "PIN_PEPPER");
      updates.deletePinChangedAt = Date.now();
    }
    await root.child(`accounts/${account.id}`).update(updates);
    await activity(account.id, action === "REMOVE" ? "DELETE_PIN_REMOVED" : "DELETE_PIN_CHANGED");
    res.json({ok: true, enabled: action !== "REMOVE"});
  }));
