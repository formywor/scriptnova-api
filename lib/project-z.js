"use strict";
const accountDevices = require("./account-devices");
...
const SEARCH_ENGINES = Object.freeze({
  ...
  snova: {label: "Snova", home: "https://api.scriptnovaa.com/search", search: "https://api.scriptnovaa.com/search?q="},
});
...
function productChoice(value) {
  ...
  if (!["share", "z", "galaxy"].includes(product)) reject("Unknown browser product.");
  return product;
}
function accountDevice(data, accountId, deviceHash) {
  ...
  const selectedId = accountDevices.find(account, data.devices, accountId, deviceHash);
  const device = data.devices?.[selectedId];
...
function purchase(data, input) {
  ...
  if (unused >= 2) reject("Maximum two unused tokens across ScriptNovaa browser products.");
  ...
  data.tokens[tokenId] = {
    ...
    accessMode: product === "galaxy" ? (option.accessMode || "single") : null,
...
function activate(data, input) {
  const {accountId, deviceHash, tokenHash, sessionId, secretHash, webAccessHash, requestId, now, mode, searchEngine, connection} = input;
  const account = accountDevice(data, accountId, deviceHash);
  const selectedDeviceId = accountDevices.find(account, data.devices, accountId, deviceHash);
  if ((account.fraudStatus || "CLEAR") !== "CLEAR") reject("Your account is under review.", 403);
  ...
  data.sessions[sessionId] = {product: "z", browser: "project-z", accountId, tokenId,
    deviceId: selectedDeviceId, sessionSecretHash: secretHash, webAccessHash, requestId,
  ...
  Object.assign(token, {status: "ACTIVE", sessionId, deviceId: selectedDeviceId, activatedAt: now, expiresAt});
...
function heartbeat(data, input) {
  ...
  if (!account || (account.accountStatus || "ACTIVE") !== "ACTIVE" || account.activeSessionId !== sessionId ||
      !accountDevices.ids(account).includes(session.deviceId) || !device || device.accountId !== accountId ||
