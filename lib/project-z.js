...
function accountDevice(data, accountId, deviceHash) {
  ...
  if (!account || (account.accountStatus || "ACTIVE") !== "ACTIVE") reject("Account not available.", 403);
...
function purchase(data, input) {
  ...
  if (account.fraudStatus !== "CLEAR") reject("Your account is under review.", 403);
...
function heartbeat(data, input) {
  ...
  if (!account || account.accountStatus !== "ACTIVE" || account.activeSessionId !== sessionId ||
